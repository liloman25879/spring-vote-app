...
# Configuration GitHub pour le stockage (à configurer avec vos credentials)
GITHUB_CONFIG = {
    "owner": "votre_username_github",  # À remplacer
    "repo": "spring-vote-data",        # À remplacer  
    "token": "votre_token_github",     # À remplacer
    "branch": "main"
}

# Descriptions réelles des tâches (identique à la version locale)
...
def github_api_request(method, endpoint, data=None):
    ...
    url = f"https://api.github.com/repos/{GITHUB_CONFIG['owner']}/{GITHUB_CONFIG['repo']}/contents/{endpoint}"
    headers = {
...
def load_data():
    """Charge les données de votes, utilisateurs et tâches depuis GitHub"""
...
def load_csv_data():
    """Charge les données CSV depuis GitHub ou localement."""
    try:
        # URL Raw directe (plus simple et fiable)
...
def main():
    ...
    if st.secrets.get("use_github", False):
        ...
        st.warning("💻 Mode Local - Configurez les secrets pour le mode cloud")
    ...
    if df is None:
        st.error("Impossible de charger les données. Vérifiez la configuration.")
        return
    
    # Obtenir toutes les tâches (CSV + nouvelles)
    all_tasks = get_all_tasks(df, additional_tasks)
    
    # Le reste du code est identique à la version locale...
    # [Interface de vote et visualisation identiques]
    ...
    # Zone principale - Visualisation (identique à la version locale)
